...
$.when(
    ...
    if (wgNamespaceNumber === 6 || RelevantPageName.startsWith("File:")) {
        sllink = `${window.searchLinksSite ? window.searchLinksSite : wgScript}?title=Special:Search&search=insource:%22${Title}%22&profile=advanced&ns0=1&ns2=1&ns4=1&ns8=1&ns10=1&ns12=1&ns14=1&ns828=1`, sltext = wgULS("查找链入", "搜尋連入"), sltitle = wgULS("通过高级搜索查找链入", "搜尋實際連結至此的頁面");
    } else if (NS.includes(wgNamespaceNumber) || (isWhatlinkshere && !isTemplate)) {
        sllink = `${wgScript}?title=Special:Search&search=insource:%22${Title}%22+linksto:%22${encodeURIComponent(RelevantPageName)}%22&profile=advanced&ns0=1&ns4=1&ns8=1&ns10=1&ns12=1&ns14=1&ns828=1`, sltext = wgULS("查找链入", "搜尋連入"), sltitle = wgULS("通过高级搜索查找链入", "搜尋實際連結至此的頁面");
    } else if (isTemplate) {
